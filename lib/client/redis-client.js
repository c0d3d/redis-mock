 ...
 * Stream functions
 */
RedisClient.prototype.xadd = RedisClient.prototype.XADD = function () {

  var argArr = Array.prototype.slice.call(arguments);

  var key = argArr.shift();
  var sizeRestriction = null;

  if (argArr[0] === "MAXLEN") {
    sizeRestriction = {};

    // Need to parse size restriction
    sizeRestriction.type = argArr.shift();
    sizeRestriction.isExact = argArr[0] === "="; // Default is to do exact trimming
    // If non-exact was specified we need to remove it from the args.

    if (argArr[0] === "~") {
      argArr.shift();
    }

    sizeRestriction.threshold = argArr.shift();
  }

  // TODO(NR) what happens with no field/value?
  // TODO(NR) check case sensitivity
  // TODO(NR) check wrong key type
  ...
  /* At this point argArr now contains only field/value pairs. */
  this._selectedDb.xadd(
    key,
    sizeRestriction,
    id,
    argArr, // Field/value pairs
    cb
  );
};
